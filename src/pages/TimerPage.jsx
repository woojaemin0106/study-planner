  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-black px-6 py-10 font-sans selection:bg-blue-100">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8 md:mb-12 flex flex-col gap-2 transition-all">
          <h1 className="text-3xl md:text-4xl font-black text-gray-900">Timer</h1>
        ...
        {/* 타이머 표시 화면 */}
        {(isActive || timeLeft > 0) && (
          <div className="text-center py-10 md:py-16 bg-white rounded-[32px] md:rounded-[48px] border border-gray-100 shadow-sm transition-all mb-12">
            <div className="text-6xl md:text-9xl font-extralight mb-6 md:mb-10 tabular-nums tracking-tighter text-gray-900 leading-none px-4 overflow-hidden">
              {formatTime(timeLeft)}
            </div>
        ...
        <div className="w-full space-y-16">
          <section className="space-y-8 md:space-y-12">
            <h2 className="text-xl md:text-2xl font-black text-gray-900 border-b border-gray-100 pb-4">타이머 설정</h2>
                ...
                <div key={item.label} className="space-y-4 text-center">
                  ...
                  <div className="relative">
                    <select 
                      ...
                      className="w-full bg-[#f1f3f5] border-none rounded-[16px] md:rounded-[20px] p-4 md:p-6 appearance-none text-2xl md:text-4xl font-extrabold focus:ring-4 focus:ring-blue-100 transition-all text-center text-gray-900"
                    >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-y-8 md:gap-y-12 py-4 md:py-8">
            {presets.map((preset, index) => (
              <button 
                key={index} 
                onClick={() => handlePreset(preset.value)}
                className="flex items-center gap-3 text-gray-400 hover:text-blue-600 transition-all group justify-center md:justify-self-center hover:scale-110"
              >
                ...
                <span className="text-[15px] md:text-[17px] font-bold">{preset.label}</span>
              </button>
            ))}
          </div>
          ...
          <div className="pb-10 md:pb-20">
            <button 
              onClick={handleStart}
              className="w-full bg-blue-600 hover:bg-blue-700 text-white font-black py-5 md:py-7 rounded-[20px] md:rounded-[28px] transition-all text-xl md:text-2xl shadow-[0_10px_30px_rgba(37,99,235,0.2)] md:shadow-[0_20px_40px_rgba(37,99,235,0.3)] active:scale-[0.98]"
            >
              START
            </button>
          </div>
